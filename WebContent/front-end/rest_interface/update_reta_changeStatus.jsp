...
<head>
    ...
    <style type="text/css">
       ...
       font-size:14px;
        ...
        body {
			background-image:url('<%=request.getContextPath()%>/front-end/rest_interface/img/image-2.jpg');
    ...
    <div class="callout" ></div>
            ...
            <div class="row">
 			...
 			<div id="page-wrapper col-md-12">
                ...
                <ol class="breadcrumb">
                    <li>
                         <a href="<%=request.getContextPath()%>/front-end/rest_interface/listRest_idAllRetaselect.jsp">訂位管理</a>
                    </li>
							                                    ...
							                                    <table class="table table-bordered table-hover table-striped">
							                                        <thead>
																		...
																		<tr align='center' valign='middle'>
																			...
																			<td><fmt:formatDate value="${avtbSvc.getOneAvtb(retaVO.avtb_id).avtb_date_s}" pattern="yyyy-MM-dd HH:mm"/><br>
																				<fmt:formatDate value="${avtbSvc.getOneAvtb(retaVO.avtb_id).avtb_date_e}" pattern="yyyy-MM-dd HH:mm"/></td>
...
<FORM METHOD="post" ACTION="<%=request.getContextPath()%>/front-end/restaurant/reta/reta.do" name="form1">
		...
		<c:if test="${retaVO.reta_status == '0'}" var="condition" scope="page" > 
	            <h3><font color="red">未到位</font></h3>
	            </c:if>
	            <c:if test="${retaVO.reta_status  == '1'}" var="condition" scope="page" > 
	           <h3><font color="green">已到位</font></h3>
	    </c:if>
		<select name="reta_status" id="reta_status" value="<%=retaVO.getReta_status()%>">
			  <c:if test="${retaVO.reta_status == '0'}" var="condition" scope="page" > 
			  <option value="0" selected>未到位</option>
			  <option value="1">已到位</option>
			  </c:if>
			  <c:if test="${retaVO.reta_status  == '1'}" var="condition" scope="page" > 
			  <option value="0">未到位</option>
			  <option value="1" selected>已到位</option>
			  </c:if>
			</select>
