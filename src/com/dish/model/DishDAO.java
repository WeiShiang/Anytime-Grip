...
public class DishDAO implements DishDAO_interface {
	...
	private static final String INSERT_STMT = "insert into dish(dish_id,rest_id,dish_name,dish_price,dish_status,dish_detail,dish_note) "
			+ "values(dish_sq.nextval,?,?,?,?,?,?)";

	private static final String DELETE = "DELETE FROM dish where dish_id = ?";

	private static final String UPDATE = "UPDATE dish set rest_id=?, dish_name=?, dish_price=?, dish_status=?, dish_detail=?, dish_note=? where dish_id = ?";

	private static final String GET_ALL_STMT = "select * from dish order by dish_status desc";
