...
//				.getRequestDispatcher("/back-end/sysu/listAllSysu.jsp");
		...
		if ("update".equals(action) || ("updatePasswd".equals(action)) ) { // 來自update_emp_input.jsp的請求
			...
			try {
				...
				SysuVO sysuVOcurrent = sysuSvc.getOneSysu(sysu_id);
				String sysu_passwd_current = sysuVOcurrent.getSysu_passwd();
				
				String sysu_passwd = req.getParameter("sysu_passwd");
				...
				if ("updatePasswd".equals(action)) {
					...
					sysu_passwd=Integer.toString(randomPasswd);					
				}				
...
//					errorMsgs.add("沒有選擇權限:"+e.getMessage());
				...
				if ((!sysu_passwd_current.equals(sysu_passwd)) || ("updatePasswd".equals(action)) ){
					String[] args={sysu_account,sysu_passwd,sysu_lastname,sysu_firstname,sysu_email}; 
...
//------為了展示用-------
				randomPasswd=123456;
//--------------------					
				String sysu_passwd=Integer.toString(randomPasswd);
				System.out.println("SysuServlet.java line.282 New Sysu Password: "+sysu_passwd);
